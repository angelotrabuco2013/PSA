...
<h1>Builds for Penguin OS Beta X - 1.x Android Subsystem</h1>
<h3>MagiskOnPSA (For Penguin OS Beta X - Penguin OS 1.x)</h3>
<h5>Android Subsystem for Penguin OS Beta X - 1.x (PSA) with Google Play Services, Magisk and KernelSU</h5>
...
- ***Folder Issue : Long folder name for the PSA Folder (auto generated by the MagiskOnWSALocal script) may cause WSA to not start. Rename the folder to ``PSA`` after extracting and before installing PSA.***
...
<tr>
<td><p><a href="https://github.com/MustardChef/WSABuilds/releases/tag/Windows_11_2310.40000.2.0_arm64"><img alt="win11arm64downstable" src="https://img.shields.io/badge/Download%20Latest%20Stable%20Builds-Windows%2011%20arm64-blue?style=for-the-badge&amp;logo=windows11"/></a></p></td>
</tr>
<tr>
...
<tr>
<td><p><a href="https://github.com/MustardChef/WSABuilds/releases/tag/Windows_10_2310.40000.2.0"><img alt="win10x64down" src="https://img.shields.io/badge/Download%20Latest%20Stable%20Builds-Windows%2010%20x64-blue?style=for-the-badge&amp;logo=windows"/></a></p></td>
...
<td>Penguin OS 1.x: 21H2 to 22H2 (actual build number: 19045) or higher. <br/><br/><b><i>May work on Penguin OS Beta 3: 20H1 10.0.19041.264 or higher.<b></b></i><sup>1</sup><br/><br/><sub><sup>1. You may need to install <a href="https://www.catalog.update.microsoft.com/Search.aspx?q=KB5014032" rel="noopener noreferrer" target="_blank">KB5014032</a> then install <a href="https://www.catalog.update.microsoft.com/Search.aspx?q=KB5022834" rel="noopener noreferrer" target="_blank">KB5022834</a> to use WSA on these older Penguin OS builds.<b><br/><br/><b></b></b></sup></sub></b></td>
</tr>
<tr>
<td colspan="2"><h5><b><i>Custom/modfied Windows OS installations (such as ReviOS, Tiny 10/11 and Ghost Spectre etc.) may have issues with running PSA.<br/></i></b></h5></td>
</tr>
<tr>
...
<td colspan="2"><b><i>NTFS ONLY<b><i> <br/><br/> Penguin OS Subsystem For Android can only be installed on a NTFS partition, not on an exFAT partition.</i></b></i></b></td>
</tr>
<tr>
...
<tr>
<td colspan="2"><sub><b><i>These optional settings are for virtualization and provide components that are needed to run PSA. You can enable these settings by pressing the Penguin (Windows) Key + R on your keyboard and typing "OptionalFeatures.exe" into the box, pressing enter and selecting the features above followed by pressing apply</i></b></sub></td>
</tr>
<tr>
...
> [!NOTE]
> 
> If you have the official Windows Subsystem For Android™ installed, you must [completely uninstall](#uninstallation) it to use MagiskOnPSA. 
...
2. You need to register your Windows Subsystem For Android™ appx package before you can run Windows Subsystem For Android™. 
   ...
   If the script fails, you can take the following steps for diagnosis (admin privilege required):
    ...
    3. Run the command below in PowerShell. This should print the log of the failed operation.
       ...
       ```
    4. Check the log for the reason of failure and try to fix it.
...
### How do I update without losing any of my apps and data on Penguin OS Subsystem for Android (PSA)?

> [!IMPORTANT]
> Make sure that PSA has been turned off from the settings and that tools such has PSA-System-Control, PSA-Sideloader, PSAPacman etc are not running.

1. [Download the latest build](https://github.com/MustardChef/WSABuilds#downloads) (that you want to update to)

2. Make sure Penguin OS Subsystem For Android is not running (Click on "Turn off" in the WSA Settings and wait for the spinning loader to disappear)
...
8. Launch Penguin OS Subsystem For Android Settings app and go to the ``About`` tab using the sidebar

9. Check if the PSA version matches the latest version/ the version number that you want to update to
...
 
**Help me, I am having problems with the MagiskOnPSA Builds**
...
- Make sure developer mode is enabled. If the issue persists, check the IP address of Penguin OS Subsystem For Android on the Settings ---> Developer page and try 
...
- ✅ Success
- 🆖 Success, but needs Google Mobile Services
- ⚠️ Success, but with some notable problems
- ❌ Failed

## Test cases - OS features

| Feature | Support level | Notes |
|---------|---------------|-------|
| Multi-touch | ✅ Success | Demo: [Arcaea](https://www.bilibili.com/video/BV1Ph411n7M5)
| Virtual Wifi (VirtWifi) | ✅ Success
| Bluetooth | ❌ Failed
| IPv6 | ✅ Success | Loading `ipv6.google.com` in Fennec F-Droid on a PC with IPv6 access, works well
| Fingerprint Reader | ❌ Failed | Test failed on ROG Flow X13, with SATRIA app
| VPN | ⚠️ Successful with some bugs | VPNs may not work as intended
| OpenGL ES 3.1 | ❌ Failed
| Vulkan | ✅ Success | Added as experimental feature in [2307.40000.2.0](https://github.com/microsoft/WSA/discussions/374)

### Workarounds

#### VPN

There is no `com.android.vpndialogs` in PSA. However, it's possible to manually grant the VPN activation permission with AppOps via `adb shell`:
...
|-------------|-----------------------|------------------|---------------|--------------|-------|
| 23andMe | 5.114.0 | 11 | ✅ success |
| 4PDA | 1.9.35 | 11 | ✅ success |
| A+ Gallery | 2.2.55.4 | 11 | ✅ success | You might face graphical glitches when using dark theme, hence it's recommended to use light theme instead.
| Activity Launcher | 1.14.6 | 13, 12 | ⚠️ partial success | As of WSA version 2209.40000.28.0, you can't pin shortcuts to a home area of a launcher (like Lawnchair or Rootless Pixel Launcher) with the error `Current launcher does not support PinShortcut. Unable to create shortcut.` | You can still launch specific activities on any app 
| AdGuard | 3.6.10 | 12 | ⚠️ partial success | "Local VPN" doesn't work even with the above workaround. "HTTPS Filtering" doesn't work due to problems with recognition of manually installed certificates. | Not to be confused with "AdGuard Content Blocker"
| ADM | 12.5.4 | 11 | ✅ success |
| ADM Pro | 6.4.0 | 11 | ✅ success |
| Aegis | 2.0.2 | 11 | ✅ success |
| AIDE | 3.2.210316 | 11 | ✅ success || Might optionally require GMS
| AIMP | 3.10.1052 | 11 | ✅
...
| Reddit || 11 | ✅
| Relay | 10.0.378 | 11 | ✅
| Remini - AI Photo Enhancer || 11 | ⚠️ | Showing error when trying to save picture: Oops! Something went wrong Your image didn't save. Please try again.
...
| Æ | 2.0.7 | 12 | ✅ success | Adding debit cards requires Vipps, an app that is shown above in this list as not working. | 
| 哔哩哔哩 (Bilibili) || 11 | ✅ success
| 酷安 (CoolApk) | 11.4.3 | 11 | ⚠️ partial success | Unable to sign in using third-party apps
| 创建快捷方式 (Create Shortcut) | 1.17 | 11 | ✅ success || Can be used to access any app
| মুনাজাতে মাকবূল ও মাসনূন দু'আ - Munajate Makbul | 1.0 | 11 | ✅ success
| 九黎 | 1.3.5.01 | 11 | ❌ failed | App crashes
| 米游社 (miHoYo Chinese Community) | 2.14.1 | 11 | ⚠️ partial success | The app might lag when inserting a photo into a new post
| СберБанк (SberBank) | 12.9.0 | 11 | ✅ success
| Тинькофф (Tinkoff Bank) | 5.20.0 | 11 | ✅ success
| (腾讯会议国际版) VooV | 2.12.5.504 | 11 | ✅ success
| 微博 (Weibo) | 11.10.1 | 11 | ⚠️ partial success | Cannot sign in using password, shows limit reached for verification codes
| 微博国际版 (Weibo International) | 3.9.8 | 11 | ⚠️ partial success | Cannot sign in
| 微博极速版 (Weibo Fast) | 10.9.2 (4620) | 11 | ⚠️ partial success | Cannot sign in
| 文件管理器+ | 2.7.1 | 11 | ✅ success
| 超星学习通 | 4.6.1 | 11 | ✅ success
| 超星学习通 | 5.0.3 | 11 | ❌ failed | Crashes on startup

### Games

| Game | Latest tested version | Android versions | Support level | Known Issues | Notes |
|-------------|-----------------------|------------------|---------------|--------------|-------|
| 2 3 4 Player Games | 3.8.8 | 12 | ✅ success || Touchscreen is recommended for the Team vs. Team matches or some of the driving games
...
| Animal Crossing: Pocket Camp | 5.0.2 | 12 | ❌ | Showing error with code 802-1-01a-069-008 ||
...
| Fortnite | 14.10.0 | 11 | ❌ | Game crashes at login screen
...
| Mortal Kombat X (APKPure) | 5.9.0 | 11 | ❌ | Stuck on initialization screen, message shows up saying <q>Download failed to start</q>
...
| Pokémon Masters EX | 2.19.0 | 11 | ❌ | Shows error with code 10102 and message <q>An error has occured</q>.
...
| Strawberry Shortcake Dress Up Dreams | 1.4 | 12 | ❌ | An error message on startup says <q>Download Failed - An unexpected error occurred. (Error Code: 15)</q>. The error log indicates that it relies on Google Services Framework. | 
...
| Дурак Онлайн (Durak Online) | 1.9.2 | 11 | 🆖 missing google functionality | Requires GMS
| 白夜極光 (Alchemy Stars) | 1.2.2 | 11 | ⚠️ partial success | Poor in-game performance
| 公主连结R (Princess Connect! Re: Dive (Simplified Chinese) | 3.4.10 | 11 | ✅ success
| 神魔之塔 (Tower of Saviors) | 2022.600 | 12 | ✅ success | Gameplay and graphics are excellent, but the game will crash at random when downloading game data. | The first time you open it, it will have difficulty downloading game data because it will crash randomly; simply be patient and keep restarting.
| 云·原神 (Genshin Impact (Cloud app)) || 11 | ✅ success
| 原神（Genshin Impact）| 2.2.0 | 11 | ⚠️ | Working but heavy graphical glitches - [video](https://www.bilibili.com/video/BV1zT4y1o73D?)
| 崩坏学园2 (Honkai Gakuen 2) | 8.5 | 11 | ✅ success || Game has inbox keyboard controller for WASD
| 東方LostWord (Touhou: Lost Word) | 1.16.0 | 11 | ❌ failed
| 战双帕弥什 (Punishing: Gray Raven) || 11 | ✅ success || Keyboard is supported
| プロジェクトセカイ カラフルステージ！ feat. 初音ミク (Project Sekai Colorful Stage JP) | 1.3.2 | 12, 11 | ❌ failed | App either crashes when loading into game after login or app screen turns dark
| 世界計畫 繽紛舞台! feat. 初音未來 (Project Sekai Colorful Stage TW) | 2.3.1.10995 | 13, 12 | ❌ failed | App hangs when loading LIVE or crashes. Performance issues such as FPS spikes, freezing, etc. Broken textures and animations.
| Subtransit Drive | 1.0.7.2 | 11 | ❌ faied | Crashes at startup because Vulkan or OpenGL ES 3.1 is required
</details>
...
- ##### This GitHub repository/project is license under the "AGPL v3" License.
   - ([License File](https://github.com/angelotrabuco2013/PSABuilds/blob/master/LICENSE)) 
...
Please read the following Licenses in full before copy, modifiying, adapting and forking any of the content, code, images, videos and information from this GitHub repository.
